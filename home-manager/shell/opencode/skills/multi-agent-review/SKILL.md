...
3. Send the original packet verbatim and actual changed surface to `@testrunner` for targeted validation. It executes section 8: targeted TuoStudio commands through `nix develop --command`, or the relevant Nix/OpenCode evaluation/format checks. A targeted pass is not a pre-PR gate.
4. Send the original packet, executor report, validation evidence, and actual diff to the read-only `@reviewer`.
5. For sensitive scope or diff—Supabase migrations/baseline; SQL policy, RLS, RPC, function, view, grant, trigger, or security context; function auth configuration; or privileged Supabase Edge Functions—also send the diff, packet, canonical docs, and `supabase-migration-review` skill to `@database-security-reviewer`. Its `REQUEST_CHANGES` or `BLOCKED_REVIEW` blocks progress. Rerun it after each sensitive change.
6. Route validation, reviewer, security, and debugger-informed fixes only to the original implementation owner. If a PR HEAD changes, all previous review and validation evidence is stale; repeat applicable targeted validation, review, security review, and final verification for the new exact HEAD.
7. Once required review/security evidence passes and the candidate final commit/PR HEAD is fixed, send the packet verbatim to `@testrunner` in `full-pre-pr` mode. For TuoStudio it must run exactly `nix develop --command pnpm verify` against that exact HEAD. Missing, stale, or failed evidence blocks PR creation.
8. Only immediately before merging, request fresh explicit user approval for that specific PR and exact HEAD. Any changed HEAD or intervening action invalidates the approval. Max three fix rounds; then ask the user.
